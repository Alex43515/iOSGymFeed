...
// Extract the value for Lean Mass Index and return as an integer
// Extract the value for Lean Mass Index and return as an integer
Future<int> carbsScanner(String? geminiParse) async {
  // Use regex to extract the value for Lean Mass Index
  final carbsScannerMatch = RegExp(r'Carbs:\s*(\d+)').firstMatch(geminiParse!);

  // Convert the extracted value to an integer or return 0 if not found
  if (carbsScannerMatch != null) {
    final leanMassValue = double.parse(carbsScannerMatch.group(1)!);
