...
// Extract the value for Lean Mass Index and return as an integer
// Extract the value for Lean Mass Index and return as an integer
Future<int> proteinScanner(String? geminiParse) async {
  // Use regex to extract the value for Lean Mass Index
  final proteinScannerMatch =
      RegExp(r'Protein:\s*(\d+)').firstMatch(geminiParse!);

  // Convert the extracted value to an integer or return 0 if not found
  if (proteinScannerMatch != null) {
    final leanMassValue = double.parse(proteinScannerMatch.group(1)!);
