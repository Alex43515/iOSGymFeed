...
// Extract the value for Lean Mass Index and return as an integer
// Extract the value for Lean Mass Index and return as an integer
Future<int> caloriesScanner(String? geminiParse) async {
  // Use regex to extract the value for Lean Mass Index
  final caloriesScannerMatch =
      RegExp(r'Kcal:\s*(\d+)').firstMatch(geminiParse!);

  // Convert the extracted value to an integer or return 0 if not found
  if (caloriesScannerMatch != null) {
    final leanMassValue = double.parse(caloriesScannerMatch.group(1)!);
